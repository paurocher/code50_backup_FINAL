...
void tabulate(void)
{
    ...
    for (int voter = 0; voter < voter_count; voter++)
    {
        ...
        {
            // this var keeps track if the vote has already been counted. If it has, ignore this
            // vote and get to the next one.
            if (!counted_vote)
            {
                int pref = preferences[voter][pref_id];
...
bool is_tie(int min)
{
    // Get amount of votes first non disabled has because I really do not get how min would help  me
    // here ... Then I will compare it to the other non disabled candidates.
    int votes = 0;
    for (int can = 0; can < candidate_count; can++)
    {
        if (!candidates[can].eliminated)
